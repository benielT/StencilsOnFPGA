...
float test_blacksholes_call_option(BlacksholesParameter calcParam, double * time_to_run, bool verbose)
{
...
void initialize_grid(float* grid, GridParameter gridProp, BlacksholesParameter computeParam)
{
...
bool verify_grid(float* grid_1, float* grid_2, GridParameter gridData)
{
    ...
    for(unsigned int bat = 0; bat < gridData.batch; bat++)
    {
		...
		for(unsigned int i = 0; i < gridData.act_size_y; i++)
		{
			for(unsigned int j = 0; j < gridData.act_size_x; j++)
			{
				if (abs(grid_1[i*gridData.grid_size_x + j+offset] - grid_2[i*gridData.grid_size_x + j+offset]) > EPSILON)
				{
					std::cout << "VERIFICATION MISMATCH: " << " batch= " << bat << ", (x,y)= (" << j << "," << i << "), " 
							<< "grid1_val= " << grid_1[i*gridData.grid_size_x + j+offset] 
							<< ", grid2_val = " << grid_2[i*gridData.grid_size_x + j+offset] << std::endl;
					// return false;
					verificationPassed = false;
				}
				// else
				// {
				// 	std::cout << "VERIFICATION: " << " batch= " << bat << ", (x,y)= (" << j << "," << i << "), " 
				// 			<< "grid1_val= " << grid_1[i*gridData.grid_size_x + j+offset] 
				// 			<< ", grid2_val = " << grid_2[i*gridData.grid_size_x + j+offset] << std::endl;
				// }
			}
...
void print_grid(float* grid, GridParameter gridProp, const std::string gridName)
{
	std::cout << "Initial Grid Value: " << gridName << std::endl;
	std::cout << "-----------------------------------------------" << std::endl;
    ...
    for(unsigned int bat = 0; bat < gridProp.batch; bat++)
    {
		...
		for(unsigned int i = 0; i < gridProp.act_size_y; i++)
		{
			for(unsigned int j = 0; j < gridProp.act_size_x; j++)
			{
				std::cout << grid[i*gridProp.grid_size_x + j+offset] << " ";
				if (j % 8 == 0)
					std::cout << std::endl;
			}
		}
